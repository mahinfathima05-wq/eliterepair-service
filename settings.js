...
(function(){
  ...
  async function saveServer(settings){ try{ const res = await fetch('/settings', { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify(settings) }); if(!res.ok) throw new Error('Server save failed'); return await res.json(); }catch(e){ return null; } }
  async function loadServer(){ try{ const res = await fetch('/settings'); if(!res.ok) return null; return await res.json(); }catch(e){ return null; } }
